...
void main() {
  test(
    'formal longer chapter translation with confirmed style profile',
    () async {
      final File report = File(
        r'F:\vibe coding\epub-translator-flutter-clean\work\live_formal_long_report.txt',
      );
      ...
      final String mode =
          Platform.environment['LIVE_TRANSLATION_CHAPTER_MODE'] ?? 'c02';
      ...
      for (final ExtractedBlock source in selected.blocks) {
        ...
        // Rough mixed-language heuristic for prose-like blocks.
...
InspectedChapter _selectChapter(List<InspectedChapter> content, String mode) {
  ...
  if (mode == 'c02' || mode == 'target') {
    for (final String key in <String>['c02', 'c03', 'c04', 'c01']) {
      for (final InspectedChapter c in content) {
        if (c.path.toLowerCase().contains(key) && c.blocks.length >= 20) {
          return c;
        }
      }
    }
  ...
  // Fallback: longest non-trivial content chapter under 120 blocks.
  final List<InspectedChapter> candidates = sorted
      .where((InspectedChapter c) => c.blocks.length >= 30)
      .toList(growable: false);
